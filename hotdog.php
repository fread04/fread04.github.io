		...
		<div class="content">
			<div>
				<div>
					...
					<ul>
						<?php
						$hotdogs = mysqli_query($connect_menu, "SELECT * FROM `hotdogs`");
						$hotdogs = mysqli_fetch_all($hotdogs, MYSQLI_ASSOC);
						$last_index = count($hotdogs) - 1;

						foreach ($hotdogs as $index => $hotdog) {
							$class = $index === $last_index ? 'last' : '';
							?>
							<li class="<?php echo $class; ?>">
								<h2><a href="hotdog.php">
										<?php echo $hotdog['name']; ?>
									</a></h2>
								<p>
									<?php echo $hotdog['description']; ?>
								</p>
								<span class="price">
									<?php echo '₴' . $hotdog['price']; ?>
								</span>
							</li>
							<?php
						}
